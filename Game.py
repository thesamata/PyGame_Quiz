...
class GameApp:
    ...
    def load_images(self):
        try:
            ...
            while True:
                try:
                    frame = PhotoImage(file=boss_gif_path, format=f'gif -index {i}')
                    self.bossgif_image.append(frame)
                    i += 1
                except Exception:
                    break
            self.frameCnt = len(self.bossgif_image)
    ...
    def setup_launcher(self):
        ...
        self.launcher_label = Label(self.root, text="🛡️ Hoş Geldin Savaşçı 🛡️", font=("Segoe UI", 14, "bold"), fg="#FFD700", bg="#1a1a1a")
        self.launcher_label.place(relx=0.5, rely=0.1, anchor=CENTER)

        self.isim_girin_label = Label(self.root, text="İsmini Bahşet:", fg="#E0E0E0", bg="#1a1a1a", font=("Segoe UI", 10))
        self.isim_girin_label.place(relx=0.5, rely=0.35, anchor=CENTER)

        self.isim_giris = Entry(self.root, font=("Segoe UI", 10), justify=CENTER, bd=2, relief=FLAT)
        self.isim_giris.place(relx=0.5, rely=0.5, width=180, height=25, anchor=CENTER)

        self.giris_button = Button(self.root, text="Maceraya Başla", bg="#2E7D32", fg="white", font=("Segoe UI", 10, "bold"), 
                                  relief=RAISED, cursor="hand2", command=self.basla_sorgu)
        self.giris_button.place(relx=0.5, rely=0.75, width=120, height=35, anchor=CENTER)

        self.kapi_button = Button(self.root, text="ℹ️ Bilgi", bg="#455A64", fg="white", font=("Segoe UI", 8), 
                                 relief=FLAT, cursor="hand2", command=self.kapilar_bilgi)
        self.kapi_button.place(relx=0.1, rely=0.9, width=50, height=25, anchor=CENTER)
    ...
    def baslat_hikaye(self):
        ...
        self.story_window.protocol("WM_DELETE_WINDOW", self.root.quit)

        if hasattr(self, 'story_resim1'):
            Label(self.story_window, image=self.story_resim1, bg="black").pack()

        self.story_label = Label(self.story_window, fg="#E0E0E0", bg="black", width=80, height=12, font=("Georgia", 11, "italic"), wraplength=700)
        self.story_label.place(relx=0.5, rely=0.3, anchor=CENTER)
        
        self.story_button = Button(self.story_window, text="Devam Et ➔", bg="#D32F2F", fg="white", font=("Segoe UI", 11, "bold"), 
                                   width=15, relief=RAISED, cursor="hand2", command=self.hikaye_ilerlet)
        self.story_button.place(relx=0.8, rely=0.85, anchor=CENTER)
    ...
    def baslat_boss_konusma(self):
        ...
        self.boss_window.protocol("WM_DELETE_WINDOW", self.root.quit)
        ...
        self.konusma_label = Label(self.boss_window, text="Sen kimsin?", fg="#FFEB3B", bg="black", width=65, height=5, font=("Segoe UI", 12, "bold"), wraplength=600)
        self.konusma_label.place(relx=0.5, rely=0.8, anchor=CENTER)   

        self.boss_button = Button(self.boss_window, text="Cevap Ver 🗣️", bg="#616161", fg="white", font=("Segoe UI", 10, "bold"), 
                                 width=15, relief=RAISED, cursor="hand2", command=self.konusma_ilerlet)
        self.boss_button.place(relx=0.85, rely=0.9, anchor=CENTER)   
    ...
    def baslat_kat_bir(self):
        ...
        self.kat_bir_window.protocol("WM_DELETE_WINDOW", self.root.quit)

        if hasattr(self, 'katbir_resim1'):
            Label(self.kat_bir_window, image=self.katbir_resim1, bg="black").pack()

        Label(self.kat_bir_window, text=f"📍 1. KAT: {self.oyuncu_ismi}", bg="black", fg="#FFD700", font=("Segoe UI", 16, "bold")).place(relx=0.5, rely=0.08, anchor=CENTER)

        kapi_img = self.katbir_kapiresim1 if hasattr(self, 'katbir_kapiresim1') else None
        
        btn_kapi1 = Button(self.kat_bir_window, text="Soldaki Kapı", bg="#1a1a1a", fg="white", font=("Segoe UI", 10, "bold"),
                          image=kapi_img, compound=TOP, relief=FLAT, cursor="hand2", command=self.soru_goster)
        btn_kapi1.place(relx=0.25, rely=0.55, anchor=CENTER)

        btn_kapi2 = Button(self.kat_bir_window, text="Sağdaki Kapı", bg="#1a1a1a", fg="white", font=("Segoe UI", 10, "bold"),
                          image=kapi_img, compound=TOP, relief=FLAT, cursor="hand2", command=self.soru_goster)
        btn_kapi2.place(relx=0.75, rely=0.55, anchor=CENTER)
    ...
    def soru_goster(self):
        ...
        self.soru_window.protocol("WM_DELETE_WINDOW", self.root.quit)
        ...
        if self.oyun_sayac <= self.oyun_sayisi:
            ...
            # Soru Arka Plan Paneli
            q_frame = Frame(self.soru_window, bg="#000000", padx=20, pady=20)
            q_frame.place(relx=0.5, rely=0.25, anchor=CENTER)
            
            Label(q_frame, text=f"SORU {self.oyun_sayac}", font=("Segoe UI", 10, "bold"), fg="#FFD700", bg="#000000").pack()
            Label(q_frame, text=self.secilen_soru["soru"], bg="#000000", fg="white", font=("Segoe UI", 15, "bold"), wraplength=700).pack(pady=10)
            
            # Can göstergesi
            Label(self.soru_window, text=f"❤️ Can: {self.health}", font=("Segoe UI", 12, "bold"), fg="#FF5252", bg="black").place(x=20, y=20)
            
            # Şıkları yerleştir
            colors = ["#424242", "#616161", "#424242", "#616161"]
            y_pos = 400
            for i, (sik, color) in enumerate(zip(self.secilen_soru["siklar"], colors)):
                btn = Button(self.soru_window, text=sik, bg=color, fg="white", font=("Segoe UI", 11, "bold"), 
                            width=25, height=2, relief=RAISED, cursor="hand2", 
                            command=lambda idx=i: self.cevap_kontrol(idx))
                btn.place(relx=0.5, y=y_pos, anchor=CENTER)
                y_pos += 45
        else:
            Label(self.soru_window, text=f"🔥 Bitti sanıyorsan yanılıyorsun,\n{self.oyuncu_ismi}! 🔥", 
                  bg="black", fg="#FFD700", font=("Segoe UI", 18, "bold")).place(relx=0.5, rely=0.3, anchor=CENTER)
            Button(self.soru_window, text="Üst Katın Anahtarını Al", bg="#D32F2F", fg="white", font=("Segoe UI", 12, "bold"), 
                   relief=RAISED, cursor="hand2", command=self.baslat_kat_iki).place(relx=0.5, rely=0.8, anchor=CENTER)
    ...
    def cevap_kontrol(self, secilen_index):
        ...
        try:
            if is_correct:
                ...
                # Eğer yanlış sesi farklı bir dosya ise buraya ekleyin, şimdilik varsa b.mp3 kullanıyoruz
                # Eğer yanlış için ayrı bir dosya yoksa, basari.mp3 dışında bir şey çalmalı
                pygame.mixer.music.load(self.get_path("music", "b.mp3"))
        ...
        self.sonuc_window.protocol("WM_DELETE_WINDOW", self.root.quit)
        ...
        Label(self.sonuc_window, text=mssg, bg="#1a1a1a", fg="white", font=("Segoe UI", 12, "bold"), wraplength=350).place(relx=0.5, rely=0.2, anchor=CENTER)
        
        if self.health <= 0:
            Button(self.sonuc_window, text="💀 Yenildin (Çıkış)", bg="#D32F2F", fg="white", font=("Segoe UI", 10, "bold"),
                   command=self.root.quit).place(relx=0.5, rely=0.8, anchor=CENTER)
        else:
            Button(self.sonuc_window, text="Devam Et ➔", bg="#2E7D32", fg="white", font=("Segoe UI", 10, "bold"),
                   command=self.baslat_kat_bir_from_sonuc).place(relx=0.5, rely=0.8, anchor=CENTER)
    ...
    def baslat_kat_iki(self):
        ...
        self.kat_iki_window.protocol("WM_DELETE_WINDOW", self.root.quit)
